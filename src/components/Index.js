...
export class Index extends Component {
  ...
  fetcher = () => {
    // axios.get('https://oofood.herokuapp.com/api/plats')
  ...
  // add to cart
  // onAddToCart = (item) => {
  //     fetch('https://oofood.herokuapp.com/api/add/' + item.id, {
  //         method: 'PUT',
  ...
  onAddToCart = (item) => {
    axios.put(`https://oofood.herokuapp.com/api/add/${item.id}`).then((res) => {
      console.log(res);
    });
  ...
  onRemoveFromCart = (item) => {
    axios
      .put(`https://oofood.herokuapp.com/api/remove/${item.id}`)
      .then((res) => {
        console.log(res);
      });
    console.log("lahcen");
  ...
  // // remove from cart
  // onRemoveFromCart = (item) => {
  //     fetch('https://oofood.herokuapp.com/api/remove/' + item.id, {
  //         method: 'PUT',
  //         body: JSON.stringify(item),
  //         headers : {
  //             'Content-Type': 'application/json',
  ...
  platcat = (item) => {
    axios
      .get(`https://oofood.herokuapp.com/api/platscategorie/${item}`)
      .then((res) => {
        this.setState({
          pizzas: res.data,
        });
      ...
      <div>
        <Router>
          ...
          <Route
            exact
            path="/ordered"
            render={() => <Ordered onRemoveFromCart={this.onRemoveFromCart} />}
          />
          ...
          <Route
            exact
            path="/login"
            render={(props) => <Login {...props} pizzas={this.state.pizzas} />}
          />
          ...
          <Route
            exact
            path="/orderdash"
            render={(props) => <Order_dash {...props} />}
          />

          <Route
            exact
            path="/paniervide"
            render={(props) => <Paniervide {...props} />}
          />
        </Router>
      </div>
    );
